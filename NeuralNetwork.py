...
class NeuralNetwork:
    ...
    def train(self, inputs_list, targets_list):
        ...
        self.who += self.lr * numpy.dot(
                (outputs_errors * final_outputs * (1.0 - final_outputs)),numpy.transpose(hidden_outputs)
            )
        self.wih += self.lr * numpy.dot(
                (hidden_errors * hidden_outputs * (1.0 - hidden_outputs)),numpy.transpose(inputs)
            )
        print("Ошибка:", outputs_errors)
        print("Веса первого слоя:", self.wih)
        print("Веса второго слоя:", self.who)
        pass
